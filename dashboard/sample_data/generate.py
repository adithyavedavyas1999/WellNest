...
STATE_LAT_LNG = {
    ...
    "AR": (34.7, -92.4), "CA": (36.8, -119.4), "CO": (39.1, -105.4),
    "CT": (41.6, -72.7), "DE": (39.0, -75.5), "FL": (27.8, -81.8),
    ...
    "NM": (34.3, -106.0), "NY": (43.0, -75.0), "NC": (35.6, -79.0),
    "ND": (47.5, -100.5), "OH": (40.4, -82.9), "OK": (35.0, -97.1),
    "OR": (43.8, -120.5), "PA": (41.2, -77.2), "RI": (41.6, -71.5),
    "SC": (34.0, -81.0), "SD": (43.9, -99.4), "TN": (35.5, -86.6),
    "TX": (31.0, -100.0), "UT": (39.3, -111.1), "VT": (44.0, -72.7),
    "VA": (37.4, -78.2), "WA": (47.7, -120.7), "WV": (38.6, -80.6),
    "WI": (43.8, -88.8), "WY": (43.1, -107.6),
}

# State-level bias: some states tend higher/lower (loosely inspired by real data)
STATE_BIAS = {
    "MA": 8, "CT": 7, "NJ": 6, "NH": 7, "VT": 6, "MN": 5, "VA": 4,
    "MD": 3, "CO": 4, "WA": 3, "HI": 3, "UT": 3, "IA": 2, "WI": 2,
    "PA": 1, "NY": 1, "IL": 0, "OR": 1, "CA": 0, "OH": -1, "MI": -2,
    "IN": -1, "FL": -1, "TX": -1, "GA": -2, "NC": -1, "MO": -2,
    "NV": -2, "AZ": -1, "KY": -3, "TN": -2, "SC": -3, "LA": -5,
    "AR": -4, "AL": -5, "MS": -7, "WV": -5, "NM": -4, "OK": -3,
}

SCHOOL_NAME_PREFIXES = [
    "Washington", "Lincoln", "Jefferson", "Franklin", "Roosevelt", "Martin Luther King Jr.",
    "Kennedy", "Adams", "Madison", "Jackson", "Monroe", "Hamilton", "Grant", "Wilson",
    "Edison", "Emerson", "Whitman", "Longfellow", "Hawthorne", "Twain",
    "Westside", "Eastside", "Northview", "Southfield", "Lakeview", "Riverside",
    "Oakwood", "Maple", "Cedar", "Pine", "Elm", "Birch", "Willow", "Magnolia",
    "Sunrise", "Sunset", "Meadow", "Valley", "Highland", "Hillcrest", "Ridgewood",
    "Fairview", "Pleasant", "Heritage", "Liberty", "Freedom", "Unity", "Harmony",
    "Discovery", "Innovation", "Achievement", "Excellence", "Horizon", "Pioneer",
    "Centennial", "Central", "Spring", "Autumn", "Greenfield", "Brookside",
]

SCHOOL_NAME_SUFFIXES = [
    "Elementary School", "Middle School", "High School", "Academy",
    "Preparatory School", "Magnet School", "Charter School",
    "Elementary", "Primary School", "Junior High School",
]

COUNTY_NAMES = [
    "Washington", "Jefferson", "Franklin", "Lincoln", "Madison", "Jackson",
    "Marion", "Clay", "Monroe", "Grant", "Hamilton", "Greene", "Union",
    "Warren", "Clark", "Montgomery", "Adams", "Carroll", "Henry", "Marshall",
    "Lawrence", "Morgan", "Perry", "Pike", "Shelby", "Sullivan", "Wayne",
    "Lake", "Orange", "Lee", "Scott", "Douglas", "Fulton", "Hancock",
    "Knox", "Logan", "Putnam", "Russell", "Randolph", "Crawford",
    "Fayette", "Delaware", "Harrison", "Howard", "Johnson", "Mercer",
    "Noble", "Owen", "Ripley", "Spencer", "Stark", "Taylor", "Wabash",
]

CITIES = [
    "Springfield", "Franklin", "Greenville", "Bristol", "Fairview",
    "Clinton", "Madison", "Georgetown", "Arlington", "Ashland",
    "Burlington", "Chester", "Dayton", "Easton", "Farmington",
    "Glen Cove", "Hartford", "Jamestown", "Kingston", "Lexington",
    "Manchester", "Newport", "Oxford", "Plymouth", "Richmond",
    "Salem", "Troy", "Vernon", "Weston", "York",
]
...
def generate() -> None:
    print("Generating WellNest sample data...")
    ...
    for sc in scores:
        for yr in [2022, 2023, 2024]:
            ...
            trends.append({
                ...
                "composite_score": round(_clamp(sc["composite_score"] + drift + random.gauss(0, 2)), 1),
                "education_score": round(_clamp(sc["education_score"] + drift + random.gauss(0, 3)), 1),
                "health_score": round(_clamp(sc["health_score"] + drift + random.gauss(0, 3)), 1),
                "environment_score": round(_clamp(sc["environment_score"] + drift + random.gauss(0, 2)), 1),
                "safety_score": round(_clamp(sc["safety_score"] + drift + random.gauss(0, 3)), 1),
            })
    ...
    print(f"\nDone! {len(schools)} schools, {len(counties)} counties, "
          f"{len(trends)} trend rows, {len(gaps)} gaps, "
          f"{len(anomalies)} anomalies, {len(briefs)} briefs, {len(flags)} flags.")
